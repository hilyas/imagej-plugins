...
public class Filter_UnsharpMask implements PlugInFilter {
    ...
    public void run(ImageProcessor orig) {
        ...
        ImageProcessor mask = orig.duplicate();     
        ImageProcessor sharp = orig.duplicate();     
        ImageProcessor gray_mask = orig.duplicate();     
        ...
        for (int v=1; v<=h-2; v++) {
            for (int u=1; u<=w-2; u++) {
                ...
                int diff = copy.getPixel(u,v) - orig.getPixel(u,v);
                mask.putPixel(u,v,diff);
                gray_mask.putPixel(u,v,diff + 128);

              	int summ = 5*mask.getPixel(u,v) + copy.getPixel(u,v);
              	sharp.putPixel(u,v,summ);
            }
        }

        ImagePlus maskcopy = new ImagePlus("Mask image", mask);
        maskcopy.show();

        ImagePlus graymaskcopy = new ImagePlus("Gray Mask image", gray_mask);
        graymaskcopy.show();

        ImagePlus sharpcopy = new ImagePlus("Sharp image", sharp);
        sharpcopy.show();
    }
